...
//@author: [Jay](https://github.com/WqyJh)
//@object: *AwsS3
//@function: UploadBinary
//@description: Upload a url to Aws S3 using aws-sdk-go. See https://docs.aws.amazon.com/sdk-for-go/v1/developer-guide/s3-example-basic-bucket-operations.html#s3-examples-bucket-ops-upload-file-to-bucket
//@param: file io.Reader
//@param: fileName string
//@return: string, string, error

func (*AwsS3) UploadBinary(file io.Reader, fileName string) (string, string, error) {
	session := newSession()
	uploader := s3manager.NewUploader(session)

	fileKey := fmt.Sprintf("%d%s", time.Now().Unix(), fileName) // 生成文件键名，这里使用了时间戳和固定的文件名
	filename := global.GVA_CONFIG.AwsS3.PathPrefix + "/" + fileKey
